...
bool UGameplayStatics::ObjectIsA(const UObject* Object, UClass* ObjectClass) {
    return false;
}
...
APlayerState* UGameplayStatics::GetPlayerStateFromUniqueNetId(const UObject* WorldContextObject, const FUniqueNetIdRepl& UniqueId) {
    return NULL;
}

APlayerState* UGameplayStatics::GetPlayerState(const UObject* WorldContextObject, int32 PlayerStateIndex) {
    return NULL;
}
...
bool UGameplayStatics::DeprojectSceneCaptureToWorld(ASceneCapture2D* SceneCapture2D, const FVector2D& TargetUV, FVector& WorldPosition, FVector& WorldDirection) {
    return false;
}
