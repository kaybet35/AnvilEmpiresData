...
USceneComponent::USceneComponent(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer) {
    ...
    this->bShouldSnapScaleWhenAttached = false;
    ...
    this->bComputeFastLocalBounds = false;
    this->bComputeBoundsOnceForGame = false;
    this->bComputedBoundsOnceForGame = false;
    this->bIsNotRenderAttachmentRoot = false;
...
void USceneComponent::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const {
    ...
    DOREPLIFETIME(USceneComponent, bShouldSnapScaleWhenAttached);
    DOREPLIFETIME(USceneComponent, Mobility);
}
