...
//CROSS-MODULE INCLUDE V2: -ModuleName=ChaosCloth -ObjectName=ChaosClothingSimulationFactory -FallbackName=ChaosClothingSimulationFactory
...
USkeletalMeshComponent::USkeletalMeshComponent(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer) {
    ...
    this->bUpdateMeshWhenKinematic = false;
    this->bUpdateJointsFromAnimation = false;
    this->bAllowClothActors = true;
    ...
    this->bForceCollisionUpdate = false;
    ...
    this->bPropagateCurvesToFollowers = false;
    ...
    this->BodySetup = NULL;
    this->ClothMaxDistanceScale = 1.00f;
    this->ClothingSimulationFactory = UChaosClothingSimulationFactory::StaticClass();
...
bool USkeletalMeshComponent::GetTransformAttribute_Ref(const FName& BoneName, const FName& AttributeName, FTransform& OutValue, ECustomBoneAttributeLookup LookupType) {
    return false;
}

bool USkeletalMeshComponent::GetTransformAttribute(const FName& BoneName, const FName& AttributeName, FTransform DefaultValue, FTransform& OutValue, ECustomBoneAttributeLookup LookupType) {
    return false;
}
