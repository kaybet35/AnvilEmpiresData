---@meta

---@class ABPVisTownCraftingLeather_C : AVisRefinery
---@field ExtraProps USceneComponent
---@field Audio UAudioComponent
---@field SM_Torch_01a4 UStaticMeshComponent
---@field SM_Torch_01a3 UStaticMeshComponent
---@field SM_Torch_01a2 UStaticMeshComponent
---@field SM_Torch_01a5 UStaticMeshComponent
---@field SM_Torch_01a1 UStaticMeshComponent
---@field Niagara UNiagaraComponent
---@field SM_Torch_01a UStaticMeshComponent
---@field MainStructureCollapsedMesh UVisStaticMeshComponent
---@field PropsRefining UVisStaticMeshComponent
---@field Props UVisStaticMeshComponent
---@field MainStructureMesh UVisStaticMeshComponent
---@field Niagara1 UNiagaraComponent
---@field StaticMesh11 UStaticMeshComponent
---@field StaticMesh13 UStaticMeshComponent
---@field StaticMesh12 UStaticMeshComponent
---@field StaticMesh6 UStaticMeshComponent
---@field StaticMesh10 UStaticMeshComponent
---@field StaticMesh9 UStaticMeshComponent
---@field StaticMesh2 UStaticMeshComponent
---@field StaticMesh8 UStaticMeshComponent
---@field StaticMesh7 UStaticMeshComponent
ABPVisTownCraftingLeather_C = {}



