---@meta

---@class UBPOptionsMenuKeybindWidget_C : UOptionsMenuKeybindWidget
---@field Action UBPAnvilKeyEntryWidget_C
---@field Action_1 UBPAnvilKeyEntryWidget_C
---@field Aim UBPAnvilKeyEntryWidget_C
---@field AltAttack UBPAnvilKeyEntryWidget_C
---@field AltShield UBPAnvilKeyEntryWidget_C
---@field ArmPrimaryItem UBPAnvilKeyEntryWidget_C
---@field ArmSecondaryItem UBPAnvilKeyEntryWidget_C
---@field BuildMenu UBPAnvilKeyEntryWidget_C
---@field CenterMap UBPAnvilKeyEntryWidget_C
---@field CinematicMode UBPAnvilKeyEntryWidget_C
---@field Climb UBPAnvilKeyEntryWidget_C
---@field DisableSnapping UBPAnvilKeyEntryWidget_C
---@field Drop UBPAnvilKeyEntryWidget_C
---@field EnterTextChat UBPAnvilKeyEntryWidget_C
---@field Inventory UBPAnvilKeyEntryWidget_C
---@field Map UBPAnvilKeyEntryWidget_C
---@field Mount UBPAnvilKeyEntryWidget_C
---@field MouseInteract UBPAnvilKeyEntryWidget_C
---@field MouseSelect UBPAnvilKeyEntryWidget_C
---@field MoveBack UBPAnvilKeyEntryWidget_C
---@field MoveForward UBPAnvilKeyEntryWidget_C
---@field MoveLeft UBPAnvilKeyEntryWidget_C
---@field MoveRight UBPAnvilKeyEntryWidget_C
---@field Push UBPAnvilKeyEntryWidget_C
---@field PushtoTalk UBPAnvilKeyEntryWidget_C
---@field Reload UBPAnvilKeyEntryWidget_C
---@field RotateCamera UBPAnvilKeyEntryWidget_C
---@field RotateCameraLeft UBPAnvilKeyEntryWidget_C
---@field RotateCameraRight UBPAnvilKeyEntryWidget_C
---@field RotateStructure UBPAnvilKeyEntryWidget_C
---@field ToggleVoiceChat UBPAnvilKeyEntryWidget_C
---@field Upgrade UBPAnvilKeyEntryWidget_C
---@field UseInteract UBPAnvilKeyEntryWidget_C
local UBPOptionsMenuKeybindWidget_C = {}



