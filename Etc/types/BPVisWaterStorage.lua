---@meta

---@class ABPVisWaterStorage_C : AVisStructure
---@field BPFoliageCullStaticMeshComponent UBPFoliageCullStaticMeshComponent_C
---@field BPLandscapeCullStaticMeshComponent UBPLandscapeCullStaticMeshComponent_C
---@field WaterNode UStaticMeshComponent
---@field StartStraightMesh UStaticMeshComponent
---@field PoolWater UStaticMeshComponent
---@field PoolWaterLevel UVisCanalWaterControllerComponent
---@field CanalWaterLevel UVisCanalWaterControllerComponent
---@field AngleBase UVisStaticMeshComponent
---@field BalljointTrim UVisStaticMeshComponent
---@field AngleTrimRight UVisStaticMeshComponent
---@field AngleTrimLeft UVisStaticMeshComponent
---@field BalljointBase UVisStaticMeshComponent
---@field AdapterStart UStaticMeshComponent
---@field VisBalljointStart UVisBalljointComponent
---@field Pool UVisStaticMeshComponent
---@field Base USceneComponent
local ABPVisWaterStorage_C = {}



