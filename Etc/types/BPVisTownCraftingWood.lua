---@meta

---@class ABPVisTownCraftingWood_C : AVisRefinery
---@field VisStaticMesh15 UVisStaticMeshComponent
---@field VisStaticMesh14 UVisStaticMeshComponent
---@field VisStaticMesh16 UVisStaticMeshComponent
---@field SharpeningWheelMesh UVisStaticMeshComponent
---@field RefineSound UAudioComponent
---@field WoodPile3Mesh UVisStaticMeshComponent
---@field VisStaticMesh6 UVisStaticMeshComponent
---@field VisStaticMesh9 UVisStaticMeshComponent
---@field SawbuckMesh UVisStaticMeshComponent
---@field VisStaticMesh10 UVisStaticMeshComponent
---@field WoodPile2Mesh UVisStaticMeshComponent
---@field WoodPile1Mesh UVisStaticMeshComponent
---@field Niagara1 UNiagaraComponent
---@field VisStaticMesh13 UVisStaticMeshComponent
---@field FirepirtMesh UVisStaticMeshComponent
---@field VisStaticMesh8 UVisStaticMeshComponent
---@field ExtraProps USceneComponent
---@field MainStructureCollapsedMesh UVisStaticMeshComponent
---@field MainStructureMesh UVisStaticMeshComponent
ABPVisTownCraftingWood_C = {}



